...
spec = do
  ...
  describe "condensedFormula" $
    it "N(CH3)3" $
      parse (condensedFormula <* eof) "" "N(CH3)3" `shouldBe` Right (CondensedFormula {getCondensedFormula = [Left (MolecularFormula {getMolecularFormula = fromList [(N,1)]}),Right ([MolecularFormula {getMolecularFormula = fromList [(C,1)]},MolecularFormula {getMolecularFormula = fromList [(H,3)]}],3)]})
  describe "mol" $
  ...
  describe "con" $
    it "QuasiQuoter for CondensedFormula should produce the correct value" $
      [con|CH3(CH3)2CH3|] `shouldBe`
      CondensedFormula { getCondensedFormula =
        [
          Left MolecularFormula { getMolecularFormula = fromList [(C, 1)] }
        , Left MolecularFormula { getMolecularFormula = fromList [(H, 3)] }
        , Right (
          [
            MolecularFormula { getMolecularFormula = fromList [(C, 1)] }
          , MolecularFormula { getMolecularFormula = fromList [(H, 3)] }
          ], 2)
        , Left MolecularFormula { getMolecularFormula = fromList [(C, 1)] }
        , Left MolecularFormula { getMolecularFormula = fromList [(H, 3)] }
        ]}
